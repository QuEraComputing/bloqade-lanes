...
// Test-only: no production path runs a duration-weighted objective, but the
// admissibility and objective-contract suites need a genuinely non-uniform one.
#[cfg(test)]
mod weighted_duration;
#[cfg(test)]
pub(crate) use weighted_duration::WeightedDuration;
