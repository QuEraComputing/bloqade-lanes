//! [`MoveGenerator`] wrapper around [`ExhaustiveExpander`].
...
/// Thin wrapper that adapts an [`ExhaustiveExpander`] to the [`MoveGenerator`]
/// trait, converting the legacy `(MoveSet, Config, f64)` output into
/// [`MoveCandidate`] values.
pub struct ExhaustiveGenerator<'a> {
    expander: ExhaustiveExpander<'a>,
}

impl<'a> ExhaustiveGenerator<'a> {
    ...
    pub fn new(
        ...
        Self {
            expander: ExhaustiveExpander::new(index, blocked, max_x_capacity, max_y_capacity),
        }
    }
}

impl MoveGenerator for ExhaustiveGenerator<'_> {
    fn generate(
        &self,
        config: &Config,
        _node_id: NodeId,
...
mod tests {
    ...
    fn generator_matches_expander() {
        ...
        // New generator path.
        let generator = ExhaustiveGenerator::new(&index, std::iter::empty(), None, None);
