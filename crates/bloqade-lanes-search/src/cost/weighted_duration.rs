//! [`WeightedDuration`], a duration-weighted [`Objective`]. Compiled for tests
//! only; see its doc.
...
/// exercised by tests, and is compiled only for them.
pub(crate) struct WeightedDuration {
...
impl WeightedDuration {
    ...
    pub(crate) fn new(index: &LaneIndex, tau: f64) -> Self {
        assert!(
