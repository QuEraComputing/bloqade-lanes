...
mod tests {
    ...
    /// Expected failures: what vihaco's frame model makes true, asserted
    /// against the pinned vihaco 0.4.1, where it is not.
    ///
    /// vihaco #110 (merged 2026-09-22, not in 0.4.1) gives each function a
    /// locals region below its operands: `local_count` slots, parameters
    /// first, zero-filled at entry, which `store` no longer grows and operand
    /// ops cannot consume. Each test here asserts the behaviour that model
    /// gives, and fails today.
    ///
    /// They are marked `should_panic` with today's *exact* failure, not
    /// `#[ignore]`, so the marking is strict: once a test stops failing that
    /// way, it fails, rather than passing unnoticed. When one does, delete its
    /// attribute. Getting there takes more than the version bump — `resolve`
    /// must compute `local_count` (#110's `call` rejects one below the arity)
    /// and `simulate_stack` must seed it; see #1038.
    ///
    /// The same release also retypes the CPU (vihaco #85: untyped `u64`
    /// words, so `load u64, 0` becomes `load_u64 0`). The old spelling then
    /// fails to parse, with a message that does not match either — so a syntax
    /// port cannot be mistaken for the fix.
    mod after_vihaco_frame_model {
        use super::*;
        use crate::isa::machine::{LanesMachine, Stopped};

        /// A scratch local is reserved and zeroed at entry, so reading it
        /// before any `store` is fine. Today it validates clean — the
        /// simulation cannot tell — and fails at run time: the slot does not
        ...
        /// A local has its own slot below the operands, so storing to one
        /// cannot clobber a value already pushed. Today local 0 *is* the
        /// bottom of the frame — the zone — so `store 0` overwrites it and
        /// `cz` gets an `i64`: a validation error, and a run-time one.
        #[test]
        #[should_panic(expected = "TypeMismatch { pc: 4, expected: 5, got: 1 }")]
