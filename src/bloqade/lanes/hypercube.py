...
def partition_grid(sites: Grid, num_x: int, num_y: int):
    ...
    for by in range(num_y):
        ...
        for bx in range(num_x):
            ...
            row_blocks.append(
                Block(sites=sites[x_start:x_end, y_start:y_end].positions)
            )
...
def holobyte_geometry(
    ...
    if num_compute_bytes_x != num_mem_bytes_x:
        raise ValueError(
            "Compute sites and memory sites must have the same number of holobytes in x dimension"
        )

    if num_mem_bytes_y % num_compute_bytes_y != 0:
        raise ValueError(
            "Memory sites must have an integer multiple of holobytes in y dimension compared to compute sites"
        )
    ...
    for mem_y_id, mem_row in enumerate(memory_blocks):
        cache_y_id = (
            mem_y_id % num_compute_bytes_y
        )  # linearly map memory rows to cache rows
