...
The cost is that stack ops have to be explicitly collapsed when lowering `stack_move` into the downstream target dialects (`move`, `ilist`, etc.): `Dup` and `Swap` map their result SSA values back to their inputs, `Pop` produces nothing. This is a trivial per-statement transformation in `stack_move2move` — but it's a step that the mainstream treatment avoids by never introducing the statements in the first place.
