...
class CandidateScorer:
    ...
    def _distance_to_target(
        ...
        """Blended hop + time distance using the precomputed DistanceTable.

        Matches `crates/bloqade-lanes-search/src/heuristic.rs:176-194`
        `blended_distance()` exactly. Occupancy is intentionally ignored
        (this is the heuristic estimate, not a legality check).
        """
        if current == target_loc:
            return 0.0
        table = tree.distance_table(
            targets=frozenset(self.target.values()) | {target_loc},
            w_t=self.params.w_t,
        )
        return table.distance(current, target_loc, self.params.w_t)
    ...
    def _mobility_at(
        ...
        for lane in tree.outgoing_lanes(position):
            ...
            if dst in occupied:
                continue
            d_after = table.distance(dst, target_loc, self.params.w_t)
    ...
    def score_moveset(
        ...
        for qid, src, dst in moved_qubits:
            ...
            # Rust `entropy.rs:score_moveset` for now; change both sides together.
