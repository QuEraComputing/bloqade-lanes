...
@ir.dialect_group(kernel.union([gemini_common.dialects.movement]))
def movement_kernel(self):
    """Gemini logical kernel with user-directed atom movement.

    Use instead of @kernel when your kernel calls movement.move_to(...).
    """
    address_analysis = address.AddressAnalysis(dialects=self)
    ...
    def run_pass(
        ...
        *,
        ...
        if verify:
            ...
            from ..common.validation.move_to import MoveToValidation
            from .validation.clifford.analysis import (
                GeminiLogicalValidation,
            )
            from .validation.measurement.analysis import (
