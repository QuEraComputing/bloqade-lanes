...
class GeminiLogicalSimulatorTask(Generic[RetType]):
    ...
    _post_processing: PostProcessing[RetType] = field(repr=False)
...
class GeminiLogicalSimulator:
    ...
    def task(
        ...
        compiled_task = compile_task(logical_kernel, m2dets=m2dets, m2obs=m2obs)
        return GeminiLogicalSimulatorTask(
            compiled_task.logical_squin_kernel,
            self.noise_model,
            compiled_task.physical_arch_spec,
            compiled_task.physical_move_kernel,
            compiled_task.post_processing,
        )
